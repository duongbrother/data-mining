    ...
    private static void classifyWithThreeDifferentStrategies(List<String> dataNames) {
        ...
        for (String dataName : dataNames) {
            ...
            try {
                ...
                int threshold[] = new int[]{30, 40, 50, 60};
                for (int i = 0; i < threshold.length; i++) {
                    ClassificationResult resultWithNaiveRankTrainSet = classifyKNNWithNaiveRankTrainSet(ds, new Double(threshold[i] * ds.getTrainData().size() / 100d).intValue());
        ...
        LatexUtil.buildNaiveRankSummaryTable(results);
    }
